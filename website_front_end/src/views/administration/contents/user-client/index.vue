    ...
    <div id="add_table">
      <Table no-data-text="暂无数据" height="535" max-height="540" border :columns="columns" :data="data">
        ...
        <template slot-scope="{ row, index }" slot="state">
          <Button :type="row.state === 1?'warning':'success'"
        ...
        <template slot-scope="{ row, index }" slot="action">
          ...
          <Button :disabled="operatorRole !== 'root'" type="error" size="small" @click="remove(index)">删除
            <Icon type="md-trash"/>
          </Button>
        </template>
      </Table>
...
export default {
  ...
  watch: {
    ...
    keyword: function (newVal) {
      if (newVal === '') {
        this.api.userSearchAllClientUser(1, 10, newVal).then(rs => {
  ...
  methods: {
    searchForm() {
      this.api.userSearchAllClientUser(1, 10, this.keyword).then(rs => {
        this.data = rs.data.data.records
        this.total = rs.data.data.total + 1
      }).catch(err => {
        console.log(err);
    ...
    onPage(page) {
      sessionStorage.setItem('clientUserPage', page);
      location.reload();
    }
  },
  created() {
    this.operatorRole = JSON.parse(sessionStorage.getItem('website-user-info')).role
    if (sessionStorage.getItem('clientUserPage')) {
      this.page = parseInt(sessionStorage.getItem('clientUserPage'))
    }
    this.api.userSearchAllClientUser(this.page, 10, '').then(rs => {
      this.data = rs.data.data.records
      this.total = rs.data.data.total + 1
    }).catch(err => {
      console.log(err);
