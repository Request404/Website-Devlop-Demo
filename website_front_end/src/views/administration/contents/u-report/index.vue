...
export default {
  ...
  watch: {
    ...
    keyword: function (newVal) {
      if (newVal === '') {
        this.api.uReportGetAllReports(1, 10, newVal).then(rs => {
    ...
    handle: function (newVal) {
      if (newVal === '2') {
        this.api.uReportGetAllReports(1, 10, '').then(rs => {
          this.data = rs.data.data.records
          this.total = rs.data.data.total + 1
        }).catch(err => {
          console.log(err);
        ...
        this.api.uReportGetHandlerReport(1, 10, parseInt(newVal)).then(rs => {
          this.data = rs.data.data.records
          this.total = rs.data.data.total + 1
        }).catch(err => {
          console.log(err);
  ...
  methods: {
    searchForm() {
      this.api.uReportGetAllReports(1, 10, this.keyword).then(rs => {
        this.data = rs.data.data.records
        this.total = rs.data.data.total + 1
      }).catch(err => {
        console.log(err);
    ...
    show(index) {
      this.$Modal.info({
        ...
        content: `<b>ID：</b>${this.data[index].id}<br>
          ...
          <b>评论编号：</b>${this.data[index].commentId}<br>
          <b>举报类型：</b>${this.data[index].reportType}<br>
    ...
    remove(index) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        this.api.uReportRemove(this.data[index].id).then(rs => {
          if (rs.data.data === 1) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    onPage(page) {
      sessionStorage.setItem('uReportPage', page);
      location.reload();
    }
  },
  created() {
    this.operatorUserRole = JSON.parse(sessionStorage.getItem('website-user-info')).role
    if (sessionStorage.getItem('uReportPage')) {
      this.page = parseInt(sessionStorage.getItem('uReportPage'))
    }
    this.api.uReportGetAllReports(this.page, 10, '').then(rs => {
      this.data = rs.data.data.records
      this.total = rs.data.data.total + 1
    }).catch(err => {
      console.log(err);
