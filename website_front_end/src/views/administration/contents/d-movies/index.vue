...
export default {
  ...
  watch: {
    ...
    keyword: function (newVal) {
      if (newVal === '') {
        this.api.dMovieGetAllMovies(1, 10, newVal).then(rs => {
  ...
  methods: {
    searchForm() {
      this.api.dMovieGetAllMovies(1, 10, this.keyword).then(rs => {
        this.data = rs.data.data.records
        this.total = rs.data.data.total + 1
      }).catch(err => {
        console.log(err);
    ...
    show(index) {
      this.$Modal.info({
        ...
        content: `<b>ID：</b>${this.data[index].id}<br>
          <b>电影名：</b>${this.data[index].name}<br>
          <b>别名：</b>${this.data[index].alternateName}<br>
          <b>题材类型：</b>${this.data[index].type}<br>
          <b>制片地区：</b>${this.data[index].country}<br>
          <b>发行日期：</b>${this.data[index].releaseDate}<br>
          <b>导演：</b>${this.data[index].director}<br>
          <b>编剧：</b>${this.data[index].writer}<br>
          <b>演员表：</b>${this.data[index].actors}<br>
          <b>时长（分钟）：</b>${this.data[index].minutes}<br>
          <b>封面地址：</b>${this.data[index].coverUrl}<br>
          <b>操作时间：</b>${this.data[index].operationTime}<br>
          <b>操作人：</b>${this.data[index].operationUser}<br>
          <b>电影简介：</b>${this.data[index].introduction.substring(0, 100) + "……"}<br>`
      })
    },
    remove(index) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        this.api.dMovieRemove(this.data[index].id).then(rs => {
          if (rs.data.data === 1) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    onPage(page) {
      sessionStorage.setItem('dMoviesPage', page);
      location.reload();
    }
  },
  created() {
    if (sessionStorage.getItem('dMoviesPage')) {
      this.page = parseInt(sessionStorage.getItem('dMoviesPage'))
    }
    this.api.dMovieGetAllMovies(this.page, 10, '').then(rs => {
      this.data = rs.data.data.records
      this.total = rs.data.data.total + 1
    }).catch(err => {
      console.log(err);
