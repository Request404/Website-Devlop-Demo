<template>
  <span id="update_drawer_pane">
    <Button :disabled="formData.role === 'root'" style="margin-right: 5px" type="success" size="small" @click="drawerOpen = true">修改
     <Icon type="md-create"/>
    </Button>
  <Drawer
      :title="'管理员信息修改（编号：'+formData.id+'）'"
      v-model="drawerOpen"
      width="720"
      :mask-closable="false"
      :styles="styles"
        ...
        <Row :gutter="32">
          <Col span="12">
            <FormItem  label="用户名" label-position="top">
              <Input disabled v-model="formData.username" placeholder="管理员用户名"/>
            </FormItem>
          </Col>
          <Col span="12">
  ...
  methods: {
    update() {
      ...
      if (reg.test(this.formData.email)) {
        if (this.notNull) {
          this.api.userAdminUserInfoUpdate(this.formData).then(rs => {
            if (rs.data.data === 1) {
              this.$message({
                type: 'success',
                message: '修改成功!'
