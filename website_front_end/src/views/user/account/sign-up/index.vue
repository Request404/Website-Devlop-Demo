<template>
  <div id="sign_up_framework" :style="'background-image: url('+ bkImgUrl +')'">
    ...
    <div v-if="step === 'sendCode'" class="form-pane">
      ...
      <button @click="sendCode" :class="emailPassVerify?'sign-up sign-up-active':'sign-up'">
        {{ $t('account.sendVerifyCode') }}
      </button>
      <div class="other-choose">
        <p @click="routerTo('/account/sign-in')">{{ $t('account.hasAccount') }}</p>
      </div>
    </div>
    <div v-else-if="step === 'signUp'" class="form-pane">
      <h1>{{ $t('account.signUp') }}</h1>
      <div ref="username" class="input-frame">
        <label class="input-label" for="account_username">
          <span>{{ $t('account.signUpUsername') }}:</span>
          <input v-model="username" class="input-pane" id="account_username" type="text">
        </label>
      </div>
      <div ref="password" class="input-frame">
        <label class="input-label" for="account_password">
          <span>{{ $t('account.password') }}:</span>
          <input v-model="password" class="input-pane" id="account_password" type="password">
        </label>
      </div>
      <div ref="confirm" class="input-frame">
        <label class="input-label" for="account_confirm">
          <span>{{ $t('account.confirm') }}:</span>
          <input v-model="confirm" class="input-pane" id="account_confirm" type="password">
        </label>
      </div>
      <div ref="verifyCode" class="input-frame">
        <label class="input-label" for="account_verify_code">
          <span>{{ $t('account.verifyCode') }}:</span>
          <input maxlength="6" v-model="verifyCode" class="input-pane" id="account_verify_code" type="text">
        </label>
      </div>
      <div class="agreement-checkbox">
        <check-box :checked="checked" @Check="Checked">
          <template v-slot:label>
            {{ $t('account.agree') }}
          </template>
        </check-box>
        <a href="#" style="font-size: 1rem">《{{ $t('account.agreement') }}》</a>
      </div>
      <button @click="signUp" :class="infoPassVerify?'sign-up sign-up-active':'sign-up'">{{ $t('account.beginSignUp') }}</button>
      <div class="other-choose">
        <p @click="routerTo('/account/sign-in')">{{ $t('account.hasAccount') }}</p>
...
import bkImgUrl from '@/assets/img/account/sign-up/background-img-ai.png';
import finishImg from '@/assets/img/account/sign-up/finish.gif';
...
export default {
  ...
  data() {
    return {
      logoImg, langSvg, bkImgUrl, finishImg, arrowSvg,
      step: 'sendCode',
      email: '',
      username: '',
      password: '',
  ...
  methods: {
    ...
    sendCode() {
      if (this.emailPassVerify) {
        this.api.userVerifyCodeOne(this.email, this.utils.getDeviceInfo()).then(res => {
    ...
    signUp(){
      if(this.infoPassVerify){
        ...
        console.log('hi');
        this.api.userRegister(user).then(rs=>{
          if(rs.data.code === 200){
            this.$Message.info({
              content: '注册成功',
              duration: 10,
            ...
            this.step = 'finish'
          }else if(rs.data.code === 1010){
            this.username = this.username+ Math.round(Math.random() * 9999)
            this.$Message.warning({
              content: '用户名已被注册，尝试使用以下用户名：'+this.username,
              duration: 5,
              closable: true
            });

          }else if(rs.data.code === 1015){
            this.$Message.error({
              content: '验证码错误',
              duration: 5,
              closable: true
            });
          }else if(rs.data.code === 1012){
            this.$Message.error({
              content: '该邮箱已被注册',
              duration: 5,
              closable: true
  ...
  updated() {
    ...
    let usernameRegExp = new RegExp("^[0-9a-zA-Z_]{1,}$");
    this.emailPassVerify = emailRegExp.test(this.email);
    if (this.username === '') {
      this.$refs.username.style.borderColor = "rgba(22, 22, 22, .8)";
      this.usernamePassVerify = false
    } else if ((usernameRegExp.test(this.username) && this.username.length >= 5)) {
      this.usernamePassVerify = true
      this.$refs.username.style.borderColor = "rgba(12, 175, 77, .88)";
    } else {
      this.usernamePassVerify = false
      this.$refs.username.style.borderColor = "rgba(204, 41, 0,.88)";
    }
    if (this.password === '') {
    ...
    this.infoPassVerify = this.usernamePassVerify && this.passwordPassVerify && this.confirmPassVerify && this.verifyCodePassVerify && this.checked;
  }
}
</script>
