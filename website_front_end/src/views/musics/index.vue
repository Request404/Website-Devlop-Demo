    ...
    <div class="home-page">
      <div class="home-page-container">
        ...
        <div class="content-card">
          <h2>{{ $t('page.allMusics') }}</h2>
          <div class="content-item">
            <ul v-if="productData.length>0" class="content-ul">
              <li @click="routerToProductPage(item.id)" class="content-li"
                  v-for="(item,index) in productData" :key="index"
                  :style="'background-image: url('+item.albumAvatarUrl+')'">
...
export default {
  ...
  methods: {
    ...
    search(keyword) {
      this.api.dMusicClientGetAllMusics(0, 0, keyword).then(rs => {
        this.productData = rs.data.data;
      })
    }
  },
  created() {
    this.api.dMusicGetWebsiteIndexMusics(1).then(rs => {
      this.carouselData = rs.data.data;
    })
    this.api.dMusicClientGetAllMusics(0, 0, 0).then(rs => {
      this.productData = rs.data.data;
    })
  }
}
